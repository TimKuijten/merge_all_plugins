 ...
 * Description: Sube un CSV de contactos, filtra (dropdowns multi-select con casillas), genera correo con IA, previsualiza y envía desde el admin de WordPress. Dedup, persistencia local y descarga de borradores .EML incluidos.
 * Version:     1.7.4
 * Author:      Tim Kuijten
 */
...
// ===============================
const STORAGE_KEY = 'kt_abm_state_v9';
...
function ktParseCSV(text){
  ...
  if (headerLooksValid) {
    ...
    header = ['email','first_name','surname','country','city','role'];
    dataRows = rows;
  }
...
let FILTERS = { roles: [], countries: [], cities: [] };
...
function loadSaved(){
  try{
    ...
    KT_CONTACTS = Array.isArray(st.contacts) ? st.contacts : [];
    KT_SELECTED = new Set(Array.isArray(st.selectedKeys) ? st.selectedKeys : []);
...
function saveState(){
  try{
    ...
    const state = {
      contacts: KT_CONTACTS,
      selectedKeys: Array.from(KT_SELECTED),
...
function ktApply(){
  ...
  KT_VIEW = KT_CONTACTS.filter(c => {
    ...
    const hit = !q || [c.email, fullName, c.country, c.city, c.role]
    ...
    return hit && inRole && inCountry && inCity;
  });
  ktRenderTable();
  saveState();
}
...
(function init(){
  ...
  const csvCard = document.querySelector('.kt-card');
  if(csvCard){
    ...
    const wrap = csvCard.querySelector('.kt-row');
...
<?php
            ...
            <div class="kt-grid kt-cols-3">
                ...
                <div class="kt-card">
                    ...
                    <div class="kt-grid kt-cols-4">
                        <div>
                            <label>Nombre</label>
                            <input id="ktQ" class="kt-input" type="text" placeholder="Buscar por nombre, email, país, ciudad o rol" oninput="ktApply()" />
                        </div>
                        ...
                        <div>
                            ...
                            <div id="ddRole" class="kt-dd" data-key="roles">
                                ...
                                <div class="kt-dd-menu">
                                    <input class="kt-dd-search" placeholder="Buscar rol..." oninput="ktDD.filter('ddRole', this.value)">
            ...
            <div class="kt-grid kt-cols-3">
                <div class="kt-card" style="grid-column: span 2;">
                    <table class="kt-table">
                        <thead>
                            <tr>
                                <th></th><th>Email</th><th>Nombre</th><th>Apellido</th><th>País</th><th>Ciudad</th><th>Rol</th>
                            </tr>
                        </thead>
    ...
    public function ajax_generate() {
        ...
        $fallback = [
            'subject'   => '{{first_name}}, nota rápida para un/a {{role}} en {{city}}',
            'body_html' => 'Hola {{first_name}},<br><br>Te escribo desde Kovacic Talent. Por tu experiencia como <strong>{{role}}</strong> en {{city}}, {{country}}, creo que esto puede interesarte.<br><br>'.esc_html($prompt).'<br><br>Si te encaja, responde a este correo y te cuento más.<br><br>Saludos,<br>{{sender}}'
        ];
        ...
        $sys = "Eres un redactor de emails de Kovacic Executive Talent Research. Devuelve un JSON con las claves 'subject' y 'body_html'. ".
               ...
               "Nombre = {{first_name}}, Apellido = {{surname}}, País = {{country}}, Ciudad = {{city}}, Proceso = {{role}}, El remitente = {{sender}}. ".
    ...
    public function ajax_send() {
        ...
        foreach ($recipients as $r) {
            ...
            if ($ok) $sent++; else $errors[] = ['email'=>$email, 'error'=>$last_error ?: 'wp_mail falló'];
            usleep(250000);
        }
